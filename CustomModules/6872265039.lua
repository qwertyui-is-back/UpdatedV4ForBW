...
local function getcustomassetfunc(path)
	if not betterisfile(path) then
		...
		local req = requestfunc({
			Url = "https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/main/"..path:gsub("cat/assets", "assets"),
			Method = "GET"
		})
...
task.spawn(function()
	local function createannouncement(announcetab)
		...
		vapeiconicon.Image = getsynasset("cat/assets/VapeIcon.png")
	...
	task.spawn(function()
		pcall(function()
			if not isfile("cat/Profiles/bedwarsdata.txt") then
				local commit = "main"
				...
				writefile("cat/Profiles/bedwarsdata.txt", game:HttpGet("https://raw.githubusercontent.com/7GrandDadPGN/VapeV4ForRoblox/"..commit.."/CustomModules/bedwarsdata", true))
			end
			local olddata = readfile("cat/Profiles/bedwarsdata.txt")
			...
			repeat
				...
				if newdata ~= olddata then
					...
					olddata = newdata
					writefile("cat/Profiles/bedwarsdata.txt", newdata)
				end
