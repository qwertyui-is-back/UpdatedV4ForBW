...
local function getSpeed(normalSpeed)
	normalSpeed = normalSpeed or 23
	if lplr.Character then
		...
		if store.grapple > tick() then
			normalSpeed = normalSpeed + 60
		end
		if store.scythe > tick() then
			normalSpeed = normalSpeed + 19
		end
		if lplr.Character:GetAttribute("GrimReaperChannel") then
			normalSpeed = normalSpeed + 12
		end
			...
			if callback then
				...
				RunLoops:BindToHeartbeat("Fly", function(delta)
					...
					if entityLibrary.isAlive then
						...
						local flyVelocity = entityLibrary.character.Humanoid.MoveDirection * (FlyMode.Value == "Normal" and getSpeed(FlySpeed.Value) or 20)
				...
				RunLoops:BindToHeartbeat("InfiniteFly", function(delta)
					...
					if entityLibrary.isAlive then
						if isnetworkowner(oldcloneroot) then
							...
							local flyVelocity = entityLibrary.character.Humanoid.MoveDirection * (InfiniteFlyMode.Value == "Normal" and getSpeed(InfiniteFlySpeed.Value) or 20)
			...
			if callback then
				RunLoops:BindToHeartbeat("Speed", function(delta)
					...
					if entityLibrary.isAlive then
						...
						local speedValue = getSpeed(SpeedValue.Value)
					...
					controlmodule.moveFunction = function(Self, vec, facecam, ...)
						if entityLibrary.isAlive then
							...
							if plr then
								...
								local factor = getSpeed(23) > 23 and 6 or 4
									...
									if pos then
										...
										RunLoops:BindToHeartbeat("AntiVoid", function(dt)
											if entityLibrary.isAlive and entityLibrary.character.Humanoid.Health > 0 and isnetworkowner(entityLibrary.character.HumanoidRootPart) and (entityLibrary.character.HumanoidRootPart.Position - pos).Magnitude > 1 and AntiVoid.Enabled and lplr:GetAttribute("LastTeleported") == lastTeleport then
												...
												local realnewpos = CFrame.new(newpos == newpos and entityLibrary.character.HumanoidRootPart.CFrame.p + (newpos * ((getSpeed(3)) * dt)) or Vector3.zero)
...
run(function()
    local disabler = GuiLibrary.ObjectsThatCanBeSaved.CatV5Window.Api.CreateOptionsButton({
        Name = "MinicopterDisabler",
        Function = function(callback)
            if callback then
				if workspace.minicopter then
					warningNotification("Cat V5", "Disabling Anticheat..", 0.35)
					workspace.minicopter.Seat:Destroy()
					task.wait(0.5)
					workspace.minicopter.Driver:Destroy()
					warningNotification("Cat V5", "Disabled Anticheat!", 5)
					disabler.ToggleButton(false)
				else
					warningNotification("Cat V5", "No minicopter found!", 10)
					disabler.ToggleButton(false)
				end
            else
                print("ok")
